...
```
┌─────────────────────────────────────────────────────────────────────────────────────────┐
│  研发仓库 Push/Commit（GitHub）→ 触发内网镜像构建 pipeline                                 │
└─────────────────────────────────────────────────────────────────────────────────────────┘
                                        │
                                        ▼
┌─────────────────────────────────────────────────────────────────────────────────────────┐
│  【DevOps 对接】 从内网镜像构建 pipeline 获取 commit（仅需 GitHub 必要数据）→ 产出 JSON → 触发  │
│  数据处理在此流程中产出并传递 listener_output.json；格式见 2.2.1、2.4 节                      │
└─────────────────────────────────────────────────────────────────────────────────────────┘
                                        │
                                        ▼
┌─────────────────────────────────────────────────────────────────────────────────────────┐
│  【内网 CI/DevOps 编排】 接收 listener_output → 按规则编排 A/B 并行 → C → 部署 → 执行 → D      │
└─────────────────────────────────────────────────────────────────────────────────────────┘
                                        │
                    ┌───────────────────┼───────────────────┐
                    ▼                   ▼                   ▼
...
└─────────────────────────────────────────────────────────────────────────────────────────┘
                                        ...
                                        ▼
┌─────────────────────────────────────────────────────────────────────────────────────────┐
│ 【自动化部署】按 commit/branch 拉取内网构建产物（镜像/包）→ 部署至测试环境 → 健康检查就绪      │
└─────────────────────────────────────────────────────────────────────────────────────────┘
                                        │
                                        ▼
┌─────────────────────────────────────────────────────────────────────────────────────────┐
...
#### 2.2.1 DevOps 对接方式（推荐：内网部署）

从**内网镜像构建 pipeline** 获取 commit 数据，仅需 GitHub 上的必要字段；数据处理在此流程中产出并传递 `listener_output.json`，供下游 pipeline 消费。

**DevOps 对接流程**：

```
内网镜像构建 pipeline 触发（含 commit 信息）
                        ↓
               DevOps 对接从该 pipeline 获取 commit 数据
               （仅需 GitHub 必要数据：repo、branch、commit_sha、changed_files 等）
                        ↓
               数据处理：path_scope_mapping + path_to_api 匹配
                        ↓
               产出 listener_output.json，传递至下游 pipeline
                        ↓
               触发内网流水线（A、B 并行 → C → 拉包部署 → 执行器 → D）
```

**从内网镜像构建 pipeline 获取的 GitHub 必要数据**：`repo`、`branch`、`commit_sha`、`commit_message`、`changed_files`（path、status）；若有 `before` 可计算 diff。数据处理后产出 `listener_output.json`，格式见 2.4 节。

**实现说明**：在 `kweaver-github-build` 中，主控 workflow（`test-pipeline/commit-e2e-main-workflow.yaml`）在收集到 `commit_info.json`、`changed_files.json` 后，由独立 job **GenerateListenerOutput** 调用 `test-pipeline/scripts/generate_listener_output.py`，读取上述两个 JSON，规范化 status（如 M→modified、A→added、D→removed），生成 `change_summary`，并可选根据仓库内的 `path_scope_mapping.yaml` 匹配得到 `scopes`、`suggested_suites`、`affected_api_names`、`affected_api_paths`、`need_add_cases`，最终写出 `listener_output.json` 并与 commit-info 一并发布为 artifact。

#### 2.2.2 内网 CI/DevOps 编排步骤

编排由**内网 CI/DevOps** 完成，规则驱动。流程固定、可预测、易调试。

| 步骤 | CI/DevOps 动作 |
|------|----------------|
| 1. 接收 | 从 DevOps 获取 listener_output.json（文件、MQ、API 等） |
...
#### 2.2.3 备选：GitHub Actions 编排

若研发与测试均在 GitHub 上、且被测环境可达，可用 GitHub Actions 替代内网 DevOps 编排。提交监听 job 产出 artifact，下游 job 按 2.2.2 步骤编排。详见《提交监听服务_基于GitHub实现》第四节。

#### 2.2.4 多模块提交且存在依赖时的部署与测试
...
| 阶段 | 输入 | 输出 |
|------|------|------|
| **DevOps 对接** | 内网镜像构建 pipeline 提供的 commit 数据（仅需 GitHub 必要字段：repo、branch、commit_sha、commit_message、changed_files） | listener_output.json：数据处理后产出并传递，含变更路径、scopes、suggested_suites、affected_api_*、need_add_cases（格式见 2.4 节） |
| **内网 CI 编排** | DevOps 产出的 listener_output.json | 按规则编排：need_add_cases 则调 A；A、B 并行；汇聚后调 C；**拉包部署**；按 C 的 batches 跑 pytest；最后调 D |
...
**内网镜像构建 pipeline 需提供的 GitHub 必要数据**：`repo`、`branch`、`commit_sha`、`commit_message`、`changed_files`（path、status）；若有 `before` 可计算 diff。DevOps 对接从该 pipeline 获取后，用 path_scope_mapping、path_to_api 做匹配，产出 listener_output.json 并传递至下游。

### 2.4 listener_output 格式（DevOps / 提交监听产出）

DevOps 对接或提交监听服务产出的**标准化 JSON**，供智能体 A/B/C 消费。格式约定如下：
...
| 提交监听/DevOps 输出 | 返回格式见 **2.4 节**；DevOps 对接时由 DevOps 产出 listener_output.json；需稳定提供：变更路径、变更类型、affected_api_names、affected_api_paths、scope、suggested_suites、need_add_cases，以便智能体 A/B/C 消费 |
| 执行顺序 | 若分多批次（先功能再回归再维度），流水线需支持多轮调用 get_cases + pytest，并合并 Allure/JUnit 结果供智能体 D 使用；智能体 C 输出格式见 3.3 节 |
| **多模块部署顺序** | 当一次提交涉及多模块且存在依赖时，需维护**模块依赖配置**（如 `module_dependencies.yaml` 或 `deploy_order`），按拓扑序分层部署；详见 **2.2.4 节** |
...
| **多模块部署某层失败** | 该层及依赖其的后续层标记失败，跳过测试执行；或仅对已就绪层对应的 scope 做有限回归（见 2.2.4 节） |
...
- **本架构**：多智能体分工、测试维度分析、执行优先级、报告生成；**DevOps 对接**（2.2.1）或 CI/Workflow 编排（2.2.2）串联全流程；**规范速查**：listener_output 格式（2.4）、智能体 B/C/D 输出格式（3.2/3.3/3.4）、path_scope_mapping（3.6）、tags 约定（4.4）、报告路径（4.3）、异常降级（4.5）。
